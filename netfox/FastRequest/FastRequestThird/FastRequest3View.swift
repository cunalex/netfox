                    ...
                    .navigationDestination(isPresented: $showIntermediateScreen) {
                        ...
                        if index != 0 {
                            if let obj = model?.gap?.objecs[(model?.gap?.orderIndex ?? 1) - 1] {
                                InterScreen(showNextScreen: (model?.gap?.orderIndex == nil || (model?.gap?.orderIndex ?? 0) == 0) ? .constant(false) : $showNextScreen, isSubscriptionActive: $isSubscriptionActive, isDisabled: $isDisabled, model: model, currentTariff: currentTariff, scanObject: obj, scanTitle: model?.gap?.title ?? "", secureScreenNumber: model?.gap?.orderIndex ?? 0,
